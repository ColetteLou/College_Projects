// week 20.cpp : Defines the entry point for the console application.
//
...
void findRecord(int id)
{
	...
	while (current != NULL)
	{
		if (current->ID == id)
		{
			printf("ID: %d\n Surname: %s\n Firstname: %s\n Result:\n English<%c>\n Maths<%c>\n French<%c>\n Philosophy<%c>\n", current->ID, current->surname, current->firstname, current->english, current->maths, current->french, current->philosophy);
			return;
		}
